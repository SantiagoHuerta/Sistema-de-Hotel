...
public class Admin extends Usuario {
    ...
    /**
     * Metodo que genera un nuevo usuario de tipo Conserje. Arroja excepcion, por lo tanto debe ser manejada
     * apropiadamente.
     *
     * @param scanner El scanner necesario para el ingreso de datos por teclado
     * @param hotel   El hotel que contiene datos de conserjes ya existentes, necesario para verificar si ya existen
     *                conserjes con un ID ya existente
     * @return retorna un nuevo Conserje
     * @throws Exception si detecta un ID ya existente
     */
    public Conserje altaConserje(Scanner scanner, Hotel hotel) throws Exception {
        ...
        while (!userConfirm.equals("s") && !userConfirm.equals("S")) {
            ...
            if (hotel.existeConserje(id)) {
                throw new Exception("El ID ya existe. No se puede crear conserje con este mismo ID.\n");
            }
        ...
        Conserje nuevo = new Conserje(id, new Password(psw), nombre);

        System.out.println("Conserje creado.");
        System.out.println("Desea habilitar a este nuevo conserje ahora mismo? (s/n)");
        userConfirm = scanner.nextLine();
    ...
    /**
     * Cambia el estado de un conserje. Notese que cada vez que se crea un Conserje mediante el metodo 'altaConserje'
     * este siempre esta deshabilitado para funcionar, por ende este metodo debe ser llamado por el admin para que
     * el conserje en cuestion tenga la capacidad de funcionar sin restricciones.
     *
     * @param hotel   El hotel que contiene la informacion de los conserjes
     * @param scanner El scanner necesario para el ingreso de datos por teclado
    ...
    public void modificarDatosConserje(Hotel hotel, Scanner scanner) {

        System.out.println("Ingrese el ID del conserje que desea modificar: ");
        try {
            Conserje aModificar = obtenerConserjePorClave(scanner, hotel);
    ...
    /**
     * Elimina un conserje del sistema.
     *
     * @param hotel   El hotel que contiene la informacion de los conserjes
     * @param scanner El scanner necesario para el ingreso de datos por teclado
     */
    public void eliminarConserje(Hotel hotel, Scanner scanner) {
        try {
            Conserje aEliminar = obtenerConserjePorClave(scanner, hotel);

            System.out.println("Esta seguro que desea eliminar el siguiente conserje: \n");
            System.out.println(aEliminar);
            Menu.confirmarConTeclaS();
            String opcion = scanner.nextLine();

            if (opcion.equals("S") || opcion.equals("s")) {
                hotel.removerConserje(aEliminar.getId());
                System.out.println("El conserje se ha eliminado exitosamente.");
            } else {
                System.out.println("El conserje no se ha eliminado");
            }
        } catch (NullPointerException e) {
            System.out.println(e.getMessage());
        }
    ...
    private Conserje obtenerConserjePorClave(Scanner scanner, Hotel hotel) {
        ...
        System.out.println("Seleccione el ID del conserje: ");
